// file    : sunweb.c
// author  : rb
// purpose : SunWeb RS-485 Serial Bus SLAVE - for HotBed Devices
// date    : 180826
// last    : 190109
//
// note    : packet format:
...
void sw_handle_probe (void)
{
  ...
  if ((SW_CMD_BYTE == SW_CMD_PROBE) && (SW_ID_BYTE == dev_id))
  {
...
{
  // check Rx packet for matching command & Device ID 
  if ((SW_CMD_BYTE == SW_CMD_ATTACH) && (SW_ID_BYTE == dev_id))
  {
...
// handle normal commands - HotBed
void sw_handle_command (void)
{
  // Rx packet not for this Device
  if ((SW_ID_BYTE != SW_ID_BCASTHEAT) && (SW_ID_BYTE != dev_id))
    return;
  ...
  {
    // ----- HotBed commands -----
    // set temperature setpoint HotBed heater [100*oC]
    case HB_CMD_SETTEMP:
    {
      uint16_t temp = ((uint16_t)rx_buf[4] << 8) + rx_buf[5];

      pid_set_target (temp);
      break;
    }

    // get setpoint & actual HotBed temperature [100*oC]
    // order: Ttarget / Tactual / Tambient
    case HB_CMD_GETTEMP:
    {
      uint8_t buf[6];
      uint16_t t_set, t_act, t_amb;

      // get actual state HotBed
      t_set = (uint16_t)(pid_get_target () * 100);         // HotBed target temperature
      t_act = (uint16_t)(pt_get_temp (SENSOR_BED) * 100);  // HotBed actuel temperature
      t_amb = (uint16_t)(pt_get_temp (SENSOR_AMB) * 100);  // HotBed ambient temperature

      // swap endianness
      buf[0] = (uint8_t)((t_set & 0xff00) >> 8);
      buf[1] = (uint8_t)((t_set & 0x00ff));
      buf[2] = (uint8_t)((t_act & 0xff00) >> 8);
      buf[3] = (uint8_t)((t_act & 0x00ff));
      buf[4] = (uint8_t)((t_amb & 0xff00) >> 8);
      buf[5] = (uint8_t)((t_amb & 0x00ff));

      // & send back to Master
      sw_txpack (dev_id, HB_CMD_GETTEMP, 6, buf);
      sw_txsend ();
      break;
    }

    // get LED intensity, fan speed & PID active state
    case HB_CMD_GETSTATE : 
    {
      ...
      buf[0] = pwm_get_leds ();
      buf[1] = pwm_get_fan ();
      buf[2] = pid_get_active ();

      // & send back to Master
      sw_txpack (dev_id, HB_CMD_GETSTATE, 3, buf);
      sw_txsend ();
      break;
    }
    ...
    {
      uint8_t buf[6];
      uint16_t t_pt1, t_pt2, t_amb;

      // get actual state HotBed
      t_pt1 = (uint16_t)(pt_get_temp (SENSOR_PT1) * 100);  // HotBed temperature sensor 1
      t_pt2 = (uint16_t)(pt_get_temp (SENSOR_PT2) * 100);  // HotBed temperature sensor 2
      t_amb = (uint16_t)(pt_get_temp (SENSOR_AMB) * 100);  // HotBed ambient temperature

      // swap endianness
      buf[0] = (uint8_t)((t_pt1 & 0xff00) >> 8);
      buf[1] = (uint8_t)((t_pt1 & 0x00ff));
      buf[2] = (uint8_t)((t_pt2 & 0xff00) >> 8);
      buf[3] = (uint8_t)((t_pt2 & 0x00ff));
      buf[4] = (uint8_t)((t_amb & 0xff00) >> 8);
      buf[5] = (uint8_t)((t_amb & 0x00ff));

      // & send back to Master
      sw_txpack (dev_id, HB_CMD_GETPT100, 6, buf);
      sw_txsend ();
      break;
    }

    // set bottom LEDs light intensity [PWM = 0..100]
    case HB_CMD_SETLEDS:
    {
      pwm_set_leds (rx_buf[4]);
      break;
    }
    ...
    case HB_CMD_SETFAN:
    {
      pwm_set_fan (rx_buf[4]);
      break;
    }

    // start temperature PID control
    case HB_CMD_STARTPID:
    {
      pid_start ();
      break;
    }

    // stop temperature PID control
    case HB_CMD_STOPPID:
    {
      pid_stop ();
      break;
    ...
    case HB_CMD_STOPSHOW:
    {
      pwm_show_stop ();
      break;
    }

    // update Pt100 calibration parameters
    case HB_CMD_RTDERR:
    {
      uint8_t  num = rx_buf[4];
      uint16_t err = ((int16_t)rx_buf[5] << 8) + rx_buf[6];
...
void sw_txsend (void)
{
  ...
  msleep (5);                 // give Master time for bus turn-around
  rs485_dir (SW_SEND);
