// file    : rtc.c
// author  : rb
// purpose : STM32F04xx RTC routines
// date    : 171120
// last    : 180824
//
...
void rtc_update_time (void)
{
  ...
  // calculate elapsed seconds today
  rtc_convert_time ();
}

// convert time to seconds <> fix 'dawn after midnight error'
uint32_t rtc_convert_time (void)
{
  return (rtc_time.num_sec = (rtc_time.hrs*3600) + (rtc_time.min*60) + rtc_time.sec);
}

// start logging RTC time to UART
void rtc_log_start (void)
{
  log_rtc = 1;
}

// stop logging RTC time to UART
