...
// temperature parameters stored in user FLASH (last entry magic number)
int32_t ntc_parms[NTC_NUM+1] __attribute__((section (".userflash"))) __attribute__ ((aligned (4))) = {0, 1234};

// init FLASH
void init_flash (void)
{
  // erase FLASH page when magic is zero-filled (after burning firmware)
  if (ntc_parms[NTC_NUM] != 1234)
  {
    // magic number not correct, erase FLASH page 
    flash_erase_page (USERFLASH_BASE);   
    debug2 (FLASH_DEBUG, "#NTC offset parameters magic missing, erase FLASH page\n");
  }
  else
    debug2 (FLASH_DEBUG, "#NTC offset parameters not empty nor erased, ok\n");
}

// write NTC offset errors to FLASH
void flash_write_ntcerr (int32_t *buf, uint8_t len)
{
  ...
  for (int i = 0; i < len; i++)
    flash_write (&ntc_parms[i], buf[i]);

  // write magic in last location
  flash_write (&ntc_parms[NTC_NUM], 1234);
}

// read NTC offset errors from FLASH
...
void flash_dump_ntcerr (void)
{
  ...
  {
    sign = (ntc_parms[i] < 0) ? -1 : 1;
    printf2 ("E%d: %d.%01d\n", i, ntc_parms[i]/10, sign*ntc_parms[i]%10);
  }
}
