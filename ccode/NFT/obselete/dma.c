...
{ 
  ...
  DMA1_Channel1->CCR   = DMA_CCR_MSIZE_1 |       // 32-bit memory 
                         ...
                         DMA_CCR_TCIE;           // enable interrupts

  NVIC_EnableIRQ (DMA1_Channel1_IRQn);           // enable DMA1 interrupt
...
{
  printf ("-- common DMA registers --\n");
  printf ("DMA_ISR        : %08lx\n", DMA1->ISR);
  printf ("DMA_IFCR       : %08lx\n", DMA1->IFCR);
  printf ("-- DMA1 Channel1 registers --\n");
  printf ("DMA1_CH1_CCR   : %08lx\n", DMA1_Channel1->CCR);
  printf ("DMA1_CH1_CNDTR : %08lx\n", DMA1_Channel1->CNDTR);
  printf ("DMA1_CH1_CPAR  : %08lx\n", DMA1_Channel1->CPAR);
  printf ("DMA1_CH1_CMAR  : %08lx\n", DMA1_Channel1->CMAR);
}
