...
class VoltageChart (pg.PlotWidget):
  def __init__(self):
    ...
    self.setYRange (0.0, 20.0, padding=0.05)
...
class TempChart (pg.PlotWidget):
  def __init__(self):
    super().__init__()
    self.setTitle ("- Temperature & Humidity -")
    self.setBackground ((44, 44, 44))
    self.setXRange (0, MAX_LEN, padding=0)
    self.setYRange (0.0, 100.0, padding=0.05)
    self.showGrid (x=True, y=True, alpha=0.3)
    self.setLabel ("left"  , "temperature/humidity", units="oC/%RH")
    self.setLabel ("bottom", "time"                , units='s')
...
class myWindow (QMainWindow):
  ...
  #self.pcnt_val.setFixedWidth (VAL_WIDTH)
   ...
   #self.st_plot.addWidget (self.pcnt_tag)
  ...
  def reset_charts (self):
    print ("#resetting chart pointers")
